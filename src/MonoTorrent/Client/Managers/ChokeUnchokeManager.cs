...
namespace MonoTorrent.Client
{
    internal class ChokeUnchokeManager : IUnchoker
    {
        ...
        public void TimePassed()
        {
            ...
            var skipDownload = isDownloading &&
                               (owningTorrent.Monitor.DownloadSpeed <
                                owningTorrent.Settings.MaxDownloadSpeed*percentOfMaxRateToSkipReview/100.0);
            var skipUpload = !isDownloading &&
                             (owningTorrent.Monitor.UploadSpeed <
                              owningTorrent.Settings.MaxUploadSpeed*percentOfMaxRateToSkipReview/100.0);
            ...
            foreach (var connectedPeer in owningTorrent.Peers.ConnectedPeers)
            {
            ...
            else if (minimumTimeBetweenReviews > 0 &&
                     (SecondsBetween(timeOfLastReview, DateTime.Now) >= minimumTimeBetweenReviews) &&
        ...
        private readonly int minimumTimeBetweenReviews = 30;
        //seconds.  Minimum time that needs to pass before we execute a review

        private readonly int percentOfMaxRateToSkipReview = 90;
        ...
        private readonly PeerList nascentPeers = new PeerList(PeerListType.NascentPeers);
        //Peers that have yet to be unchoked and downloading for a full review period

        private readonly PeerList candidatePeers = new PeerList(PeerListType.CandidatePeers);
        //Peers that are candidates for unchoking based on past performance

        private readonly PeerList optimisticUnchokeCandidates =
        ...
        private void AllocateSlots(int alreadyUnchoked)
        {
            ...
            var availableSlots = owningTorrent.Settings.UploadSlots - alreadyUnchoked;
        ...
        private void ExecuteReview()
        {
            ...
            foreach (var connectedPeer in owningTorrent.Peers.ConnectedPeers)
            {
                if (connectedPeer.Connection != null)
                {
                    if (!connectedPeer.Peer.IsSeeder)
                    {
                        ...
                        var timeSinceLastReview = SecondsBetween(timeOfLastReview, DateTime.Now);
                        ...
                        else if (isDownloading && connectedPeer.IsInterested && connectedPeer.AmChoking &&
                                 bytesTransferred > 0)
            ...
            else
            {
                ...
                var oup = optimisticUnchokeCandidates.GetOUPeer();
                if (oup != null)
                {
            ...
            {
                var nextPeer = nascentPeers.GetNextPeer();
                if (!nextPeer.AmChoking)
                    Choke(nextPeer);
            }
            while (candidatePeers.MorePeers)
            {
                var nextPeer = candidatePeers.GetNextPeer();
                if (!nextPeer.AmChoking)
                    Choke(nextPeer);
            }
            while (optimisticUnchokeCandidates.MorePeers)
            {
                var nextPeer = optimisticUnchokeCandidates.GetNextPeer();
                if (!nextPeer.AmChoking)
        ...
        private void ExecuteTyrantReview()
        {
            ...
            foreach (var pid in sortedPeers)
            {
                ...
                if (uploadBandwidthUsed < owningTorrent.Settings.MaxUploadSpeed && pid.IsInterested)
                {
                    Unchoke(pid);
