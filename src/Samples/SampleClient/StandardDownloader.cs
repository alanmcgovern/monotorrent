...
namespace ClientSample
{
    class StandardDownloader
    {
        ...
        // Look for a directory side-by-side with the git checkout called 'TorrentResearch'. I have 20k torrents downloaded from internet archive.
        static readonly string TorrentResearchPath = Path.GetFullPath (Path.Combine ("..", "..", "..", "..", "..", "..", "..", "TorrentResearch"));
        // Easy way to configure how many to load.
        static readonly int MaxTorrents = 2000;// Int32.MaxValue;
        ...
        public async Task DownloadAsync (CancellationToken token)
        {
            ...
            // If nothing is in the local directory, just load from the research dir if it exists.
            var files = Directory.GetFiles (torrentsPath, "*.torrent");
            if (files.Length == 0 && Directory.Exists (TorrentResearchPath)) {
                files = Directory.GetFiles (TorrentResearchPath, "*.torrent");
            }

            int counter = 0;
            await Parallel.ForEachAsync (files.Take(MaxTorrents),
