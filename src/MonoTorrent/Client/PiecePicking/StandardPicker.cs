...
namespace MonoTorrent.Client
{
    ...
    {
        private static readonly Predicate<Block> TimedOut = delegate(Block b) { return b.RequestTimedOut; };
        ...
        public override int CurrentRequestCount()
        {
            return
                (int)
                    Toolbox.Accumulate(requests, delegate(Piece p) { return p.TotalRequested - p.TotalReceived; });
        }
            ...
            int startIndex, int endIndex, int count)
        {
            ...
            {
                // Request the piece
                var p = new Piece(checkIndex + i, id.TorrentManager.Torrent.PieceLength,
